...
def train(
    ...
    gradient_accumulation_steps: int = 4,  # <--- New argument for grad accumulation
    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            #
            # ---- GRADIENT ACCUMULATION MODIFICATION ----
            #
            # 1. Scale the loss by accumulation steps to average it.
            loss = loss / gradient_accumulation_steps
            # 2. Backprop on this scaled loss.
            loss.backward()

            # Only step optimizer once we've accumulated enough gradients
            if (step + 1) % gradient_accumulation_steps == 0:
                ...
                optimizer_step += 1
            #
            # ---- END GRADIENT ACCUMULATION MODIFICATION ----
            #
            ...
            # (Make sure you only evaluate after an optimizer.step() if you want consistent metrics)
            if (
                (optimizer_step > 0)
